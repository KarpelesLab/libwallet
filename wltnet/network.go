...
// bitcoinBalance queries modchain_assets with the account's xpub
// (preferred — gap-limit scan across both receive and change chains)
// or falls back to a single-address lookup when no xpub is available
// (view-only accounts). Returns the sum of unspent NATIVE outputs as
// an Amount with 8 decimals (satoshi).
//
// Earlier this path went through modchain_lookupTxoBIP32 because the
// xpub call was returning a stale 0 balance for some xpubs; the
// backend has since been fixed to merge both chains correctly, so
// modchain_assets is the simpler one-call form again.
