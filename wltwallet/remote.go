...
func remoteNew(ctx context.Context, number string) (*remoteKeyNewResult, error) {
	...
	return res, restApplyRetry(withClientID(ctx), "Crypto/WalletSign:new", "POST", rest.Param{"number": number}, &res)
}
...
func remotekeyNew(ctx context.Context, in struct {
	...
	res, err := restDoRetry(withClientID(ctx), "Crypto/WalletSign:new", "POST", rest.Param{"number": target})
	if err != nil {
		return nil, err
	}
	return res.Data, nil
...
func remoteVerify(ctx context.Context, session, code string) (*remoteKeyVerifyResult, error) {
	...
	return res, restApplyRetry(withClientID(ctx), "Crypto/WalletSign:verify", "POST", rest.Param{"session": session, "code": code}, &res)
}
...
func remotekeyValidate(ctx context.Context, in struct {
	...
	res, err := restDoRetry(withClientID(ctx), "Crypto/WalletSign:verify", "POST", rest.Param{"session": in.Session, "code": in.Code})
	if err != nil {
		return nil, err
	}
	return res.Data, nil
